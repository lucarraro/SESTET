function [t,y,Weight_Teq,Weight_input,Weight_lat,Weight_frict,Weight_dQdt,tt]=SESTET_solver_weight(parameters,params_Teq,aT,sT,tspan,y0)
%
...
if  strcmp(type_model,'Flat')
    ...
    function dy=eqs_15(t,y)
        ...
        dy(ind_reach) = weight_Teq + weight_input + weight_lat + weight_frict + weight_dQdt; %
        Weight_Teq=[Weight_Teq weight_Teq];
        Weight_input=[Weight_input weight_input];
        Weight_lat=[Weight_lat weight_lat];
        Weight_frict=[Weight_frict weight_frict];
        Weight_dQdt=[Weight_dQdt weight_dQdt];
        tt=[tt t];
    end
